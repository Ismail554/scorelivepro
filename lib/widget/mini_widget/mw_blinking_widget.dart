...
class _BlinkingWidgetState extends State<BlinkingWidget>
  ...
  void _scheduleNextBlink() {
    ...
    final idleMs    = widget.minIdleDuration.inMilliseconds +
        (idleRange.inMilliseconds * (identical(0, 0) ? 0.5 : _random())).round();
  ...
  // Replace with `Random().nextDouble()` if dart:math is already imported.
  static int _seed = 42;
  ...
  void _onStatus(AnimationStatus status) {
    if (status == AnimationStatus.completed) {
      if (mounted) setState(() { _phase = _BlinkPhase.idle; _blinkRatio = 0.0; });
      _scheduleNextBlink();
    }
  }
  ...
  double get _shadowBlur =>
      widget.shadowBlurRadius * (1.0 - _blinkRatio * 0.6);
